...
| `Archive`           | Keeps empty mounted CPT archives on their post type so PostsList shows the empty-state notice instead of falling back to page or front-page content. Seeds `$post` from the mount page so Modularity still loads archive modules when there are no posts. |
